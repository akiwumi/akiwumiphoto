  ...
  return jsonb_build_object(
    ...
    'funnels', jsonb_build_object(
      'visit_to_gallery', jsonb_build_object(
        ...
        'gallery_interactions', (select count(*) from (select session_id, min(occurred_at) as first_page_at from public.analytics_events where event_name = 'page_view' and occurred_at >= range_start and occurred_at < range_end group by session_id) v where exists (select 1 from public.analytics_events g where g.session_id = v.session_id and g.event_name in ('gallery_view', 'image_open') and g.occurred_at >= v.first_page_at and g.occurred_at < range_end))
      ),
      'gallery_to_paid', jsonb_build_object(
        ...
        'paid_orders', (select count(*) from (select session_id, min(occurred_at) as gallery_at from public.analytics_events where event_name in ('gallery_view', 'image_open') and occurred_at >= range_start and occurred_at < range_end group by session_id) g where exists (select 1 from public.analytics_events p where p.session_id = g.session_id and p.event_name = 'page_view' and p.occurred_at >= range_start and p.occurred_at < g.gallery_at) and exists (select 1 from public.analytics_events c where c.session_id = g.session_id and c.event_name = 'checkout_start' and c.occurred_at >= g.gallery_at and c.occurred_at < range_end) and exists (select 1 from public.analytics_events paid where paid.session_id = g.session_id and paid.event_name = 'payment_success' and paid.occurred_at >= g.gallery_at and paid.occurred_at < range_end and paid.occurred_at >= (select min(c2.occurred_at) from public.analytics_events c2 where c2.session_id = g.session_id and c2.event_name = 'checkout_start' and c2.occurred_at >= g.gallery_at and c2.occurred_at < range_end)))
      ),
      'visit_to_registration', jsonb_build_object(
        'visits', (select count(distinct session_id) from public.analytics_events where event_name = 'page_view' and occurred_at >= range_start and occurred_at < range_end),
        'registrations', (select count(*) from (select session_id, min(occurred_at) as registration_at from public.analytics_events where event_name = 'registration_complete' and occurred_at >= range_start and occurred_at < range_end group by session_id) r where exists (select 1 from public.analytics_events p where p.session_id = r.session_id and p.event_name = 'page_view' and p.occurred_at = (select min(p2.occurred_at) from public.analytics_events p2 where p2.session_id = r.session_id and p2.event_name = 'page_view' and p2.occurred_at >= range_start and p2.occurred_at < range_end) and r.registration_at >= p.occurred_at))
      )
    )
  );
end;
