  ...
  return jsonb_build_object(
    ...
    'funnels', jsonb_build_object(
      ...
      'visit_to_registration', jsonb_build_object(
        'visits', (select count(distinct session_id) from public.analytics_events where event_name = 'page_view' and occurred_at >= range_start and occurred_at < range_end),
        'registrations', (select count(*) from (select distinct session_id from public.analytics_events where event_name = 'registration_complete' and occurred_at >= range_start and occurred_at < range_end) r where exists (select 1 from public.analytics_events p where p.session_id = r.session_id and p.event_name = 'page_view' and p.occurred_at >= range_start and p.occurred_at < range_end))
      )
    )
  );
end;
