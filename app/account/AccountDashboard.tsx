  ...
  return (
    <div className="register-layout" style={{ alignItems: 'start' }}>
      <section className="register-panel" aria-labelledby="account-purchases-title">
        ...
        <p style={{ color: 'rgba(255,255,255,.68)', marginBottom: 24 }}>{email}</p>
        ...
        {data.orders.length === 0 ? (
          ...
          <div className="flex flex-col gap-6">
            {data.orders.map((order) => (
              <article key={order.id} style={{ borderTop: '1px solid rgba(255,255,255,.18)', paddingTop: 18 }}>
                <div className="flex flex-wrap justify-between gap-3">
                  <div><strong>{order.reference}</strong><div style={{ color: 'rgba(255,255,255,.65)', fontSize: 14 }}>{new Date(order.created_at).toLocaleDateString()}</div></div>
                  <div style={{ textAlign: 'right' }}><strong>{money((order.total_usd + order.shipping_usd) * order.exchange_rate, order.currency)}</strong><div style={{ color: '#E8001C', textTransform: 'capitalize', fontSize: 14 }}>{order.status}</div></div>
                </div>
                <ul style={{ margin: '16px 0 0', padding: 0, listStyle: 'none' }}>
                  {order.lines.map((line, index) => <li key={`${order.id}-${index}`} className="flex justify-between gap-4" style={{ padding: '8px 0', borderBottom: '1px solid rgba(255,255,255,.08)' }}><span>{lineName(line)}{line.size_name ?? line.size ? ` · ${line.size_name ?? line.size}` : ''}{line.quantity && line.quantity > 1 ? ` × ${line.quantity}` : ''}</span><span>{lineAmount(line, order) ?? 'Included'}</span></li>)}
                </ul>
                <div className="flex justify-between gap-4" style={{ marginTop: 12, color: 'rgba(255,255,255,.68)' }}><span>Shipping</span><span>{order.shipping_usd > 0 ? money(order.shipping_usd * order.exchange_rate, order.currency) : 'Free'}</span></div>
                {order.shipping_address && <div style={{ marginTop: 14, color: 'rgba(255,255,255,.68)', fontSize: 14 }}><strong style={{ color: '#fff' }}>Delivery</strong>{addressLines(order.shipping_address).map((line, index) => <div key={`${order.id}-address-${index}`}>{line}</div>)}</div>}
                <div style={{ marginTop: 12, color: 'rgba(255,255,255,.68)', fontSize: 14 }}>Dispatch status: {order.status === 'paid' ? 'Preparing for dispatch' : order.status}</div>
              </article>
            ))}
          </div>
        )}
      ...
      <aside className="flex flex-col gap-6">
        <section className="register-panel" aria-labelledby="account-registrations-title">
          ...
          <p style={{ color: 'rgba(255,255,255,.68)', lineHeight: 1.6 }}>Register a purchased print to keep its provenance and certificate with your account.</p>
          ...
          {data.registrations.length > 0 && <div style={{ marginTop: 24 }} className="flex flex-col gap-4">{data.registrations.map((registration) => <div key={registration.id} style={{ borderTop: '1px solid rgba(255,255,255,.18)', paddingTop: 14 }}><strong>{registration.artwork_title}</strong><div style={{ color: 'rgba(255,255,255,.65)', fontSize: 14 }}>{new Date(registration.created_at).toLocaleDateString()} · <span style={{ textTransform: 'capitalize' }}>{registration.status}</span></div></div>)}</div>}
        </section>
