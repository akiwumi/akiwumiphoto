...
export default async function RegistrationsPage() {
  ...
  return <main className={styles.main}><div className={styles.content}><header className={styles.header}><div><h1 className={styles.title}>Print registrations</h1><p className={styles.subtitle}>{rows?.length ?? 0} registrations · registration records and certificates</p></div><Link href="/admin/dashboard" className={styles.ghostButton}>← Dashboard</Link></header><div className={styles.contentPad}><div className={styles.panel} style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse'}}><thead><tr><th align="left">Registered</th><th align="left">Collector</th><th align="left">Photograph</th><th align="left">Purchase</th><th align="left">Receipt</th><th align="left">Certificate</th></tr></thead><tbody>{(rows ?? []).map(r=><tr key={r.id}><td>{new Date(r.created_at).toLocaleDateString('en-GB')}</td><td>{r.collector_snapshot?.first_name} {r.collector_snapshot?.last_name}<br/><small>{r.collector_snapshot?.email}</small></td><td>{r.artwork_title}</td><td>{r.purchased_from || 'Not provided'}<br/>{r.payment_method || 'Unknown'}</td><td><Link href={`/register/receipts/${r.id}`} target="_blank">View / print</Link></td><td><Link href={`/admin/dashboard/registrations/${r.id}/certificate`} className={styles.primaryButton}>Create certificate</Link>{(certificates ?? []).filter(c=>c.registration_id===r.id).map(c=><div key={c.id}><Link href={`/admin/dashboard/registrations/certificates/${c.id}`} target="_blank">Certificate {c.print_number}/{c.edition_total} · {c.status}</Link></div>)}</td></tr>)}</tbody></table></div></div></div></main>;
}
