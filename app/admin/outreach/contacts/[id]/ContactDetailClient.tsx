...
export default function ContactDetailClient({ id, contacts: initialContacts, categories }: { id: string; contacts: AddressBookContact[]; categories: OutreachContactCategory[] }) {
  ...
  async function updateCategory(categoryId: string) {
    if (!contact || contact.id.startsWith('manual-')) { setCategoryMessage('Manual contacts remain local and cannot be assigned a shared category.'); return; }
    setUpdatingCategory(true); setCategoryMessage('');
    try {
      const nextCategoryId = categoryId || null;
      const response = await fetch(`/api/admin/outreach/contacts/${contact.id}/category`, { method: 'PATCH', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ categoryId: nextCategoryId }) });
      const payload = await response.json() as { error?: string };
      if (!response.ok) { setCategoryMessage(payload.error || 'Could not update the contact category.'); return; }
      const selected = categories.find((category) => category.id === nextCategoryId);
      setContact((current) => current ? { ...current, categoryId: nextCategoryId, category: selected?.name ?? null } : current);
      setCategoryMessage('Contact category updated.');
    } catch { setCategoryMessage('Could not update the contact category. Check the server connection and try again.'); }
