...
export default function ContactsTable({ contacts }: { contacts: AddressBookContact[] }) {
  ...
  const filteredContacts = filterAddressBookContacts(visibleContacts, query, country);
  ...
  return <>
    ...
    <div className={styles.bulkToolbar}><div className={styles.filterControls}><input className={styles.input} placeholder="Search name, studio, email, or country" aria-label="Search address book" value={query} onChange={(event) => setQuery(event.target.value)} /><label className={styles.filterField}>Country<select className={styles.select} aria-label="Filter address book by country" value={country} onChange={(event) => setCountry(event.target.value)}><option value="">All countries</option>{countries.map((entry) => <option key={entry} value={entry}>{entry}</option>)}</select></label></div><span className={styles.bulkCount}>{filteredContacts.length} of {visibleContacts.length} visible · sent history protects against duplicates</span><button type="button" className={styles.smallButtonDanger} onClick={resetSent}>Reset sent history</button></div>
