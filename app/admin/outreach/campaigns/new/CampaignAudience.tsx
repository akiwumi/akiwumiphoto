...
export default function CampaignAudience({ contacts: initialContacts, categories: managedCategories, mailerHtml }: { contacts: AddressBookContact[]; categories: OutreachContactCategory[]; mailerHtml: string }) {
  ...
  return <section className={styles.panel}>
    <div className={styles.panelHead}><div><h2 className={styles.panelTitle}>Select recipients</h2><p className={styles.panelMeta}>Choose the available contacts for this campaign. Sent contacts remain disabled.</p></div><div className={styles.selectionControls}><fieldset className={styles.filterField} aria-label="Filter campaign recipients by country"><legend>Countries</legend>{countries.map((country) => <label key={country}><input className={styles.checkbox} type="checkbox" checked={selectedCountries.includes(country)} onChange={() => toggleCountry(country)} /> {country}</label>)}</fieldset><button type="button" className={styles.buttonSecondary} onClick={() => setSelectedCountries([])}>{selectedCountries.length === 0 ? 'All countries' : 'Show all countries'}</button><span className={styles.selectionCount}>{selectedCountries.length === 0 ? 'All countries' : `${selectedCountries.length} countries selected`}</span><fieldset className={styles.filterField} aria-label="Filter campaign recipients by category"><legend>Categories</legend>{categories.map((category) => <label key={category}><input className={styles.checkbox} type="checkbox" checked={selectedCategories.includes(category)} onChange={() => toggleCategory(category)} /> {category}</label>)}</fieldset><button type="button" className={styles.buttonSecondary} onClick={() => setSelectedCategories([])}>{selectedCategories.length === 0 ? 'All categories' : 'Show all categories'}</button><span className={styles.selectionCount}>{selectedCategories.length === 0 ? 'All categories' : `${selectedCategories.length} categories selected`}</span><button type="button" className={styles.buttonSecondary} disabled={!visibleAvailable.length} onClick={toggleAllVisibleAvailable}>{allVisibleAvailableSelected ? 'Deselect all' : 'Select all available'}</button><span className={styles.selectionCount}>{selected.length} selected · {visibleAvailable.length} available in view</span><span className={`${styles.chip} ${styles.chipWarn}`}>Step 1 · recipients</span></div></div>
