...
export default function SentTable() {
  ...
  return <div className={styles.tableScroll}><table className={styles.table}><thead><tr><th>Recipient</th><th>Subject</th><th>Sent</th><th>Provider record</th><th>Actions</th></tr></thead><tbody>{records.map((record) => { const contact = ADDRESS_BOOK.find((entry) => entry.id === record.contactId); const key = recordKey(record); const expanded = expandedKey === key; return <>
